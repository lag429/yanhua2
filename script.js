...
// 在文件开头添加默认配置
const DEFAULT_SHELL_SIZE = 12;  // 设置默认shell size为12
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const textElement = document.querySelector('.text-line');
...
// 修改init函数
function init() {
  // 将原来的init函数中的shell size默认值改为12
  const shellSize = DEFAULT_SHELL_SIZE;
  ...
  ['3"', '4"', '6"', '8"', '12"', '16"'].forEach((opt, i) => {
    const selected = i === 4 ? 'selected' : ''; // 12" 是第5个选项
    options += `<option value="${i}" ${selected}>${opt}</option>`;
  });
